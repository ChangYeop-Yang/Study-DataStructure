...
int main(void)
{
	...
	HInsert(&mHeap, 1);	HInsert(&mHeap, 2);	HInsert(&mHeap, 3); HInsert(&mHeap, 4);
	cout << HDelete(&mHeap) << endl;

	/* Sorting */
	int mArr[] = {1, 3, 5, 2, 6, 7 ,9};
	HeapSort(DataPrioirytComp, mArr, sizeof(mArr) / sizeof(int));
